...
export async function PATCH(req: Request, context: Context) {
  try {
    ...
    if (slot.tutor_student_id !== decoded.student_id) return new Response("Forbidden", { status: 403 });

    const updatedSlot = await prisma.tutor_slots.update({
      where: { id: slotId },
      data: {
    ...
    return Response.json({ message: "Updated successfully", updatedSlot });
  } catch (error) {
